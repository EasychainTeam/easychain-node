...
function build(osVersion, netVersion) {
  var dir = 'easychain-' + osVersion + '-' + package.version + '-' + netVersion;
  var fullpath = path.join(__dirname, 'build', dir);
  var cmds = [];
  cmds.push(format('cd %s && mkdir -p public dapps tmp logs bin', fullpath));
  cmds.push(format('cp -r package.json easychaind init proto %s', fullpath));
  if (netVersion != 'localnet') {
    cmds.push(format('sed -i "s/testnet/%s/g" %s/easychaind', netVersion, fullpath));
...
function buildSource(netVersion) {
  var dir = 'easychain-' + 'linux' + '-' + package.version + '-' + netVersion;
  var fullpath = path.join(__dirname, 'build', dir);
  return gulp.src('app.js')
    .pipe(webpack({
      output: {
